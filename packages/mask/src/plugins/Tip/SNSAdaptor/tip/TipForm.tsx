...
export const TipForm = memo(() => {
    ...
    const {
        ...
        erc721Contract,
        setErc721Contract,
    } = TipContext.useContainer()
    const { Utils } = useWeb3State()
    const selectRef = useRef(null)
    const { value: nativeTokenDetailed } = useNativeTokenDetailed()
    const [token = nativeTokenDetailed, setToken] = useState<FungibleTokenDetailed | undefined>(undefined)
        ...
        <Box className={classes.root}>
            ...
            <FormControl fullWidth>
                <Select
                    ...
                    value={receiver}
                    onChange={(e) => {
                        setReceiver(e.target.value as string)
                    }}
                    MenuProps={{
                        anchorOrigin: {
                            vertical: 'bottom',
